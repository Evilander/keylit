...
export function scalesForChord(chord) {
  ...
  // m7♭5 (half-diminished) and diminished
  const isHalfDim = cls === "min" && q.includes("♭5");
  const isFullDim = cls === "dim";
  ...
  if (isHalfDim) {
    ...
    add("lydian dominant", "smoother altered color");
  } else if (isDominant) {
    ...
    add("melodic minor", "whole-tone-ish brightness over the augmented");
  } else {
